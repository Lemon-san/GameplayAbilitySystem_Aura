...
TArray<USceneComponent*> APointCollection::GetGroundPoints(const FVector& GroundLocation, int32 NumPoints, float YawOverride)
{
	...
	TArray<USceneComponent*> ArrayPtrCopy;

	for (USceneComponent* Pt : ImmutablePts)
	{
		if (ArrayPtrCopy.Num() >= NumPoints) return ArrayPtrCopy;
		...
		UAuraAbilitySystemLibrary::GetAllLivePlayersWithinRadius(this, IgnoreActors, TArray<AActor*>(), 1500.f, GetActorLocation());
